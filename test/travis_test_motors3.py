...
from pomouse_ros.srv import TimedMotion
...
class MotorTest(unittest.TestCase):
    ...
    def test_pub_cmd_vel(self):
        ...
        self.file_check("rtmotor_raw_l0",200,"wrong left value from cmd_val")
        self.file_check("rtmotor_raw_r0",600,"wrong right value from cmd_val")
    ...
    def test_timed_motion(self):
        timed_motion = rospy.ServiceProxy('/timed_motion', TimedMotion)
        ret = timed_motion(300,-300,500)
        self.assertEqual(ret.success, True, "timed motion does not succeeded")
        with open("/dev/rtmotor0","r") as f:
            data = f.readline()
            self.assertEqual(data,"300 -300 500\n","wrong value in rtmotor0 at timed motion")
